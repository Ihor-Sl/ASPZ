...
Програма відкриває поточний каталог за допомогою opendir() і читає його вміст через readdir(). Для кожного файлу отримується детальна інформація через stat(), а права доступу обробляються з використанням бітових мас (наприклад, S_IRUSR, S_IXGRP). Форматується час зміни через localtime().
...
Програма відкриває файл (fopen()), читає його пострічково (fgets()), і виконує пошук слова за допомогою strstr(). Рядки, в яких виявлено співпадіння, виводяться на екран.
...
Програма читає файли з аргументів, і друкує їх рядки. Після кожних 20 рядків зупиняється і чекає на натискання клавіші (наприклад, через getchar()). Рахунок рядків ведеться змінною, яка обнуляється після кожної паузи.
...
Використовується рекурсивна функція, яка відкриває каталог (opendir()), перебирає елементи (readdir()), і, якщо елемент — підкаталог (перевіряється через stat() і S_ISDIR), функція викликає сама себе з новим шляхом. Обробляються шляхи з snprintf().
...
Читається вміст каталогу в масив рядків, фільтруються лише підкаталоги (через stat() + S_ISDIR), після чого виконується сортування (qsort() з strcmp() як компаратором). Вивід здійснюється після сортування.
...
Файли фільтруються за розширенням .out, потім перевіряється, чи є вони виконуваними (stat() + S_IXUSR). Для таких файлів через chmod() додається право читання для групи/інших (S_IROTH, S_IRGRP).
...
Програма читає всі файли з поточного каталогу (через readdir()), запитує підтвердження користувача для кожного файлу (y/n), і у випадку згоди видаляє файл через remove().
...
Використовується функція clock() для зчитування часу до і після блоку коду. Різниця в clock переводиться в мілісекунди (/ CLOCKS_PER_SEC * 1000.0).
...
Генератор випадкових чисел ініціалізується srand(time(NULL)). Значення у [0.0, 1.0] обчислюється як (float)rand() / RAND_MAX, а для [0.0, n] — множенням попереднього на n. Результати виводяться у циклі з printf().
